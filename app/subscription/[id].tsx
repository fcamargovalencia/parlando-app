...
type ActionModal = 'pause' | 'resume' | 'cancel' | 'bookingDetail';
...
export default function SubscriptionDetailScreen() {
  ...
  const [selectedBooking, setSelectedBooking] = useState<RoutineBookingResponse | null>(null);
  ...
  const [hasPauseTo, setHasPauseTo] = useState(false);
  const [showPauseFromPicker, setShowPauseFromPicker] = useState(false);
  const [showPauseToPicker, setShowPauseToPicker] = useState(false);

  // Cancel form
  const [cancelReason, setCancelReason] = useState('');

  // Override pickup fields
  ...
  const handlePause = useCallback(async () => {
    if (!id || !pauseFrom) return;
    setIsSubmitting(true);
    try {
      await pauseSubscription(id, pauseFrom, hasPauseTo ? pauseTo : undefined, pauseReason || undefined);
  ...
  }, [id, pauseFrom, pauseTo, pauseReason, hasPauseTo, pauseSubscription, reloadSubscription]);
  ...
  }, [id, cancelReason, cancelSubscription, router]);

  const handleCancel = useCallback(() => {
    const upcoming = bookings.find((b) => b.status === 'ACCEPTED');
    if (upcoming) {
      const diffH = (parseISO(upcoming.occurrenceDate).getTime() - Date.now()) / (1000 * 60 * 60);
      if (diffH > 0 && diffH < 24) {
        Alert.alert(
  ...
  const handleOverridePickup = useCallback(async () => {
    ...
    const diffH = (parseISO(selectedBooking.occurrenceDate).getTime() - Date.now()) / (1000 * 60 * 60);
    if (diffH < 2) {
      Alert.alert('No disponible', 'Solo puedes cambiar el punto con al menos 2 horas de anticipación.');
      return;
    }
    setIsSubmitting(true);
    try {
      const req: PickupOverrideRequest = { latitude: lat, longitude: lng, name: overrideName.trim() };
  ...
  }, [selectedBooking, overrideLat, overrideLng, overrideName, overridePickup]);
  ...
  if (loadError || !subscription) {
    return (
      <View className="flex-1 items-center justify-center bg-neutral-50 px-6">
        ...
        <Text className="text-sm text-neutral-500 text-center mt-1">{loadError}</Text>
  ...
  const { status, subscribedDays, startDate, endDate, seatsRequired,
    pickupType, customPickupName } = subscription;
  ...
  return (
    <View className="flex-1 bg-neutral-50" style={{ paddingTop: insets.top }}>
            ...
            {bookings.length === 0 && status === 'PENDING' ? (
              ...
              <RoutineCalendarView
                ...
                onPressBooking={(booking) => {
                  ...
                  setOverrideName(booking.pickupName ?? '');
                  setOverrideLat(booking.pickupLatitude?.toString() ?? '');
                  setOverrideLng(booking.pickupLongitude?.toString() ?? '');
          ...
          <View className="mx-4 mt-4 gap-2">
            {canPause && (
              <Button
                ...
                onPress={() => {
                  setPauseFrom(dateToISO(new Date()));
                  setPauseTo('');
                  setPauseReason('');
                  setHasPauseTo(false);
      ...
      >
        <View className="flex-1 justify-end" style={{ backgroundColor: Colors.overlay }}>
          <View className="bg-white rounded-t-3xl px-6 pt-5" style={{ paddingBottom: insets.bottom + 24 }}>
            ...
            <TouchableOpacity
              onPress={() => setHasPauseTo(!hasPauseTo)}
            ...
            {hasPauseTo && (
              <>
                ...
                <TouchableOpacity
                  onPress={() => setShowPauseToPicker(true)}
                  className="border border-neutral-200 rounded-xl px-4 py-3 mb-4"
                >
            ...
            <View className="flex-row gap-3">
              <Button variant="outline" className="flex-1" onPress={() => setActiveModal(null)}>
                Cancelar
              </Button>
              <Button
                className="flex-1"
          ...
          <View className="bg-white rounded-t-3xl px-6 pt-5" style={{ paddingBottom: insets.bottom + 24 }}>
            ...
            {selectedBooking && (
              <>
                ...
                {/* Override pickup — only if ACCEPTED and >= 2h away */}
                {selectedBooking.status === 'ACCEPTED' &&
                  (parseISO(selectedBooking.occurrenceDate).getTime() - Date.now()) / (1000 * 60 * 60) >= 2 && (
                ...
                <Button variant="outline" onPress={() => setActiveModal(null)}>
                  Cerrar
                </Button>
              </>
            )}
      ...
      <DatePickerModal
        ...
        onConfirm={(d) => { setPauseFrom(dateToISO(d)); setShowPauseFromPicker(false); }}
        onCancel={() => setShowPauseFromPicker(false)}
      />
      <DatePickerModal
        ...
        onConfirm={(d) => { setPauseTo(dateToISO(d)); setShowPauseToPicker(false); }}
        onCancel={() => setShowPauseToPicker(false)}
      />
    </View>
  );
}
